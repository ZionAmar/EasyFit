...
async function createMeeting(req, res, next) {
    const { name, trainer_id, date, start_time, end_time, room_id, participant_count } = req.body;

    if (!name || !trainer_id || !date || !start_time || !end_time || !room_id || !participant_count) {
    ...
    try {
        ...
        if (parseInt(participant_count) > capacity) {
            req.error = { status: 400, message: `כמות המשתתפים חורגת מהקיבולת המותרת (${capacity}) של החדר הנבחר.` };
            return next();
        }
        ...
        req.newMeeting = { id: result.insertId, name, trainer_id, date, start_time, end_time, room_id, participant_count };
        next();
    } catch (err) {
...
async function updateMeeting(req, res, next) {
    ...
    try {
        ...
        const [result] = await db.query(
            ...
            [name, trainer_id, date, start_time, end_time, room_id, participant_count, id]
        );
        ...
        req.updatedMeeting = { id, name, trainer_id, date, start_time, end_time, room_id, participant_count };
        next();
    } catch (err) {
